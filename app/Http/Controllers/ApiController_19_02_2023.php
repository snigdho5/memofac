...
class ApiController extends Controller
{
    ...
    private function meRatingShareWith($id, $user_id)
    {
        //My Rating against memo
        $data = PostRating::where(['user_id' => $user_id, 'post_id' => $id])->pluck('share_with')->first();
        if ($data) {
            return $data;
            // return round($data, 1);
        }
    ...
    public function memo_list(Request $request)
    {
        ...
        // $content = DB::select(
        //     "select mf_experiences.id, mf_experiences.category_id, y.id AS rate_id, mf_experiences.title, mf_experiences.description, mf_experiences.image, mf_experiences.top, mf_experiences.recomended from `mf_experiences` inner join ( select x.* from mf_post_ratings x inner join (select p.post_id, max(p.created_at) created_at from mf_post_ratings p group by p.post_id) t on (x.post_id = t.post_id and x.created_at = t.created_at) ) y on y.post_id = mf_experiences.id order by y.`id` desc"
        // )->paginate(20);
        $maxPage = 20;
        $content =  DB::select(DB::raw("select mf_experiences.id, mf_experiences.category_id, y.id AS rate_id, mf_experiences.title, mf_experiences.description, mf_experiences.image, mf_experiences.top, mf_experiences.recomended from `mf_experiences` inner join ( select x.* from mf_post_ratings x inner join (select p.post_id, max(p.created_at) created_at from mf_post_ratings p group by p.post_id) t on (x.post_id = t.post_id and x.created_at = t.created_at) ) y on y.post_id = mf_experiences.id order by y.`id` desc limit 20 offset " . $request->offset . ""));

        // $content = new Paginator($query, $maxPage);

        // $content = PostRating::selectRaw("mf_experiences.id, mf_experiences.category_id, mf_post_ratings.id AS rate_id, mf_experiences.title, mf_experiences.description, mf_experiences.image, mf_experiences.top, mf_experiences.recomended")
        //     ->join('experiences', 'post_ratings.post_id', '=', 'experiences.id')
        //     ->groupBy('experiences.id')
        //     ->orderBy('post_ratings.id', 'DESC')
        //     ->paginate(20);
        ...
        // $content = $content->unique('id');

        // $id_array = [];

        foreach ($content as $key1 => $row) {
            ...
            $catDetails = ExpCategory::select(['id', 'category_name', 'type', 'icon'])->where(['id' => $row->category_id, 'status' => 'Y'])->first();
            $row->category_name = $catDetails->category_name;
            ...
            $row->me_share_with = $this->meRatingShareWith($row->id, $user->id);
    ...
    // MEMO list new
    public function memo_list_new(Request $request)
    {
        $validator =  Validator::make($request->all(), [
            'token' => 'required',
        ]);
        ...
        if ($secondary_id != 0) {
            ...
            // print_r($memos);die;
            //get memo against secondary_id
            $content = Memo::selectRaw("id, category_id, title, description, image, top, recomended")
                ->where(['category_id' => $secondary_id, 'status' => 'Y'])
                ->where(['status' => 'Y'])
                // ->orderBy('exp', 'ASC')
                // ->orderBy('top', 'ASC')
    ...
    public function report_cats(Request $request)
    {
        $validator =  Validator::make($request->all(), [
            'token' => 'required',
        ]);
        ...
        $list = DB::table('report_categories')->get();
    ...
    public function report_post(Request $request)
    {
        ...
        $insArray = array(
            ...
            'other_category'   => ($request->other_category) ? $request->other_category : '',
        ...
        $ins = DB::table('reported_posts')->insert($insArray);
        return response()->json([
            'result' => 'success',
            'message' => 'You have reported for this post!',
            'content' => $ins
        ], 200);
    }
    ...
    public function block_user(Request $request)
    {
        ...
        $geBlUser = DB::table('blocked_users')->where(['user_id' => $user->id, 'blocked_user_id'   =>   $request->blocked_user_id])->first();
        ...
        if ($geBlUser) {
            $insArray = array(
                ...
                'edited_dtime'   =>   date('Y-m-d H:i:s')
            );

            // print_r($insArray);die;

            $upd = DB::table('blocked_users')->where(['id' => $geBlUser->id])->update($insArray);

            // $newData = DB::table('blocked_users')->where(['id' => $geBlUser->id])->first();

            return response()->json([
                'result' => 'success',
                'message' => ($request->status == 1) ? 'You have blocked this user!' : 'You have unblocked this user!',
                'content' => $upd
            ], 200);
        } else {
            $insArray = array(
                ...
                'blocked_user_id'   =>   $request->blocked_user_id,
                'status'     =>   $request->status,
                'added_dtime'   =>   date('Y-m-d H:i:s')
            );

            // print_r($insArray);die;

            $ins = DB::table('blocked_users')->insert($insArray);
            return response()->json([
                'result' => 'success',
    ...
    public function blocked_user(Request $request)
    {
        ...
        if ($request->blocked_user_id > 0) {
            $geBlUser = DB::table('blocked_users')->where(['user_id' => $user->id, 'blocked_user_id'   =>   $request->blocked_user_id, 'status' => 1])->get();
        } else {
            $geBlUser = DB::table('blocked_users')->where(['user_id' => $user->id, 'status' => 1])->get();
        }


        if ($geBlUser) {
            foreach ($geBlUser as $key => $value) {
                $getimg = DB::table('users')->select('name', 'image')->where(['id' => $value->blocked_user_id])->first();
                if ($getimg) {
                    $value->name = $getimg->name;
                    $value->image =  ($getimg->image != '') ? asset('public/images/' . $getimg->image) : '';
                } else {
                    $value->name = '';
                    $value->image = '';
                }
    ...
    public function report_usr_cats(Request $request)
    {
        $validator =  Validator::make($request->all(), [
            'token' => 'required',
        ]);
        ...
        $list = DB::table('report_user_cats')->get();
    ...
    public function report_usr_report(Request $request)
    {
        $validator =  Validator::make($request->all(), [
            'token' => 'required',
        ]);
        ...
        $ins = DB::table('reported_users')->insert($insArray);
        return response()->json([
            'result' => 'success',
            'message' => 'You have reported for this user!',
            'content' => $ins
        ], 200);
    }
    ...
    public function memorate(Request $request)
    {
        ...
        $rated_memo = DB::table('exp_cat')->select(['exp_cat.id', 'exp_cat.category_id', 'exp_cat.title', 'exp_cat.description', 'exp_cat.image', 'exp_cat.rating', 'post_ratings.post_id'])->join('post_ratings', 'exp_cat.id', '=', 'post_ratings.post_id')->where(['post_ratings.post_id' => $post_id])->orderBy('post_ratings.updated_at', 'DESC')->first();

        // foreach ($rated_memo as $value) {
        $rated_memo->all = $this->getRating($rated_memo->id);
        $rated_memo->known = $this->getKnownRating($rated_memo->id, $user->id);
        $rated_memo->close = $this->getCloseoneRating($rated_memo->id, $user->id);
        $rated_memo->me = $this->meRating($rated_memo->id, $user->id);
        $rated_memo->totalExp = $this->countExpercinaces($rated_memo->id);
        $rated_memo->image = $this->getFolderImageUrl($rated_memo->image, 'icon');
        ...
        $catDetails = ExpCategory::select(['id', 'category_name', 'type', 'icon'])->where(['status' => 'Y'])->whereIn('id', [$rated_memo->category_id])->get();

        $rated_memo->category = $catDetails;
        $catDetails = [];
        unset($rated_memo->category_id);
        // }
    ...
    public function time_line(Request $request)
    {
        ...
        $content = DB::table('post_details')->whereRaw('(`share_with` IN (1,2,3) AND `user_id` IN (SELECT `contact_user_id` from `mf_user_contact` where `user_id` =' . $user->id . ') OR `share_with` = 3) AND `user_id` NOT IN (SELECT `blocked_user_id` FROM `mf_blocked_users` WHERE `user_id` = ' . $user->id . '
        AND `status` = 1)')->latest()->paginate(20);
    ...
    public function deleteUser(Request $request)
    {
        ...
        PostReact::where('user_id', $user->id)->delete();
        PostWishlist::where('user_id', $user->id)->delete();
        ...
        return response()->json([
            'result' => 'success',
            'message' => 'User is deleted with its all activities!',
            'status' => 1
        ], 200);
    }
    ...
    public function otherUserCategoryDetails(Request $request)
    {
        ...
        if (!empty($ouser)) {
            ...
            foreach ($rated_memo as $value) {
                ...
                //     $category_ids = explode(",", $value->category_id);
                //     $category_ids = array_filter($category_ids);
                // }


                $catDetails = ExpCategory::select(['id', 'category_name', 'type', 'icon'])
                    ->where('id', $value->category_id)
                    // ->whereIn('id', $category_ids)
                    ->where(['status' => 'Y'])
                    ->first();
                $value->category_name = $catDetails->category_name;
                // $value->category_ids = $category_ids;
                // unset($value->category_id);
            }
    ...
    public function errorReport(Request $request)
    {
        ...
        $insArray = array(
            ...
            'userDeviceDetails'   =>   $request->userDeviceDetails,
        ...
        DB::table('prod_errors')->insert($insArray);
